...
class ECGApp(QMainWindow, PlotMixin, RecordMixin, StreamMixin, LogMixin, DetailMixin):
    ...
    def start_stream(self):
        self.csv_filename = f"ecg_record_{time.strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def on_new_data(self, value: float):
        ...
        # === Tambahkan ini untuk simpan ke CSV ===
        try:
            if hasattr(self, "csv_file") and self.csv_file:
                self.csv_file.write(f"{t:.6f},{value:.6f}\n")
        except Exception as e:
            self.log(f"[CSV ERROR] {e}")
        # ==========================================

        # jika buffer melebihi 60 detik, drop yg awal
